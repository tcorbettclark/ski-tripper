...
export type { Accommodation, LlmCacheRow, Participant, Preferences, Proposal }
...
export function computeInputHash(
  ...
  const data = {
    ...
    participants: participantPrefs.map((p) => ({
      userId: p.participant.participantUserId,
      userName: p.participant.participantUserName,
      preferences: {
...
export function buildUserPrompt(
  ...
  for (const { participant, preferences } of participantPrefs) {
    lines.push('')
    lines.push(`### ${participant.participantUserName}`)
