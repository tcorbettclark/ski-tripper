...
async function fetchLlmCache(
  ...
  const result = await db.listRows({
    ...
    queries: [
      ...
      Query.equal('type', 'preference-search'),
      Query.limit(10),
    ],
  })
...
export default async function handler({
  ...
  if (!tripId) {
    return res.json({ error: 'Missing tripId' }, 400)
  }

  log(`Generating preference search query for trip ${tripId}`)
  ...
  if (participantPrefsData.length === 0) {
    return res.json({ error: 'No participants with preferences found' }, 400)
  }

  const inputHash = computeInputHash(participantPrefsData)
  log(`Computed inputHash: ${inputHash}`)

  let cacheRows: LlmCacheRow[]
  try {
    cacheRows = await fetchLlmCache(db, tripId)
  } catch (err) {
  ...
  const userPrompt = buildUserPrompt(participantPrefsData)
  ...
  try {
    ...
    log('Preference search query generation complete')
    return res.json({ success: true })
  } catch (err) {
    const errorMsg =
